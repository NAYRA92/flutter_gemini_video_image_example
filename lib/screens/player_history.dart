...
class PlayerHistory extends StatefulWidget {
  ...
  const PlayerHistory({super.key, required this.playerName, required this.playerID});
...
class _PlayerHistoryState extends State<PlayerHistory> {
  ...
  List<OrdinalData> createPieGraph(
    String response
  ){
              ...
              builder: (context, AsyncSnapshot<QuerySnapshot> snapshot) {
                if (snapshot.connectionState == ConnectionState.waiting) {
                  return Center(
                    child: SizedBox(
                      height: 150,
                      child: Lottie.asset("assets/images/football_loading.json")),
                  );
                }
                else if(snapshot.data == null){
                  return Text("لا توجد بيانات سابقة بعد");
                }
                createPieGraph(snapshot.data!.docs[0]["analyis_data"]);
                      ...
                      child: Column(
                        children: [
                          Container(
                            ...
                            child: Center(
                              child: Text(snapshot.data!.docs[index]["analyis_type"],
                              style: TextStyle(
                                fontSize: 18,
                                color: Colors.white,
                          ...
                          AspectRatio(
                                                  aspectRatio: 16 / 9,
                                                  child: DChartPieO(
                                                    data: createPieGraph(snapshot.data!.docs[index]["analyis_data"]),
                                                    customLabel:
                                                        (ordinalData, index) {
                          ...
                          Padding(
                            ...
                            child: Text(snapshot.data!.docs[index]["analyis_data"],
                            style: TextStyle(
                              fontSize: 18,
                              fontWeight: FontWeight.w600
                            ),),
